...
const char *copyright = "2016-2017";
const char *email = "qiushuangxu@qq.com";
...
main (int argc, char **argv)
{
	...
	setlocale (LC_ALL, "");//用于设定locale
	bindtextdomain (PACKAGE, LOCALEDIR);//绑定寻找文件的路径
	textdomain (PACKAGE);//设置当前所需要而textdomain
	...
	/* initialize mysql  */
	mysql_init (&mysql);//这个函数用于分配或者初始化一个MYSQL对象，用于连接mysql服务端 if (opt_file != NULL) mysql_options(&mysql,MYSQL_READ_DEFAULT_FILE,opt_file); if (opt_group != NULL) mysql_options(&mysql,MYSQL_READ_DEFAULT_GROUP,opt_group); else mysql_options(&mysql,MYSQL_READ_DEFAULT_GROUP,"client"); if (ssl)
		mysql_ssl_set(&mysql,key,cert,ca_cert,ca_dir,ciphers); //可采用SSL建立安全连接
	/* establish a connection to the server and error checking */
	//mysql尝试与运行主机上的MySQL数据库引擎建立连接，在你能够执行需要有效mysql连接句柄的任何其他api之前
	...
	if(check_slave) {
		...
		if (mysql_field_count (&mysql) == 12) {
			...
			//作用是返回结果集中代表字段（列）的对象的数组，返回结果集中代表字段（列）的对象的数组，然后输出每个字段名称、表格和最大长度。[
